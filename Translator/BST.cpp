...
/* Constructors and destructor */
	...
	BST<ElementType>::BST(const BST<ElementType>& aBST)  {
		...
		if (aBST.elementCount()==0){
			...
			root = CopyHelper(aBST.root);
		}
		
	}

	template<class ElementType>
	ElementType* BST<ElementType>::CopyHelper(const BSTNode<ElementType>* src) const{    
		if (src == NULL){
			return NULL;
		}
		else {
			BSTNode<ElementType>* newnode = new BST<ElementType>(src->element);
			newnode->left = CopyHelper(src->left);
			newnode->right = CopyHelper(src->right);
			return newnode;
		}
	...
	//delete subtree
	template <class ElementType>
	void BST<ElementType>::remove(BSTNode<ElementType>* ptr) {
		if(ptr !=NULL){
			if(ptr->hasLeft){
				remove(ptr->left);
			}
...
/* BST Operations */
    ...
    ElementType& BST<ElementType>::retrieve(const ElementType& targetElement) const {
		...
		// cout << "going to retrieve "<< targetElement<< endl;
    ...
    ElementType& BST<ElementType>::retrieveR(const ElementType& targetElement, BSTNode<ElementType>* current) const {
		...
		// cout << "elsing" << endl;
        	if (current->element < targetElement && current->hasRight()){
            	return retrieveR(targetElement, current->right);
       		}
        	else if(current->element > targetElement && current->hasLeft()){
